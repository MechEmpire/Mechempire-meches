...
void RobotAI::Update(RobotAI_Order& order,const RobotAI_BattlefieldInformation& info,int myID)
{
	...
	*/
	...
	//躲子弹
	double b_distance=1000;
	for(int i=0;i<info.num_bullet;i++){
		if(1-myID==info.bulletInformation[i].launcherID){
			if(abs(info.bulletInformation[i].circle.x-my.x)<40||abs(info.bulletInformation[i].circle.y-my.y)<40){
				if(abs(info.bulletInformation[i].circle.x-my.x>0)){//右边
					if(abs(info.bulletInformation[i].circle.y-my.y)>0){//下方
						if(info.bulletInformation[i].vx<0&&info.bulletInformation[i].vy<0)
							order.run = 2;
					}
					else if(abs(info.bulletInformation[i].circle.y-my.y)==0){
						if(info.bulletInformation[i].vx<0)
							order.run = 1;
					}
					else{
						if(info.bulletInformation[i].vx<0&&info.bulletInformation[i].vy>=0)
				...
				else if(abs(info.bulletInformation[i].circle.x-my.x==0)){
					if(info.bulletInformation[i].vx<0){
						order.run = 3;
					}
					else{
