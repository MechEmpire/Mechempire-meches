...
struct Vector2D
{
  ...
  Vector2D(Circle c) :x(c.x), y(c.y){}
  Vector2D(RobotAI_BulletInformation c) :x(c.vx), y(c.vy){}
  ...
  inline double cosLaw(const Vector2D &v2)const;
...
inline double Vector2D::cosLaw(const Vector2D &v2)const
{
	return (this->LengthSq() + v2.LengthSq() - this->DistanceSq(v2)) / (2 * this->Length()*v2.Length());
}
