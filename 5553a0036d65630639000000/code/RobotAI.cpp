...
//	cout<<info.robotInformation[myID].engineRotation<<endl;
	...
	double angleofTwo=RadianToAngle(asin(sina));
		if (selfE.x>armyE.x)
		{
			angleofTwo=180-angleofTwo;//angleofTwo当机甲处于右边时该怎么算？180-angle还是360-angle?
		}
//	cout<<"连线角度 "<<angleofTwo<<"  ";
	
	double angleW=info.robotInformation[myID].weaponRotation;
//	cout<<"武器角度 "<<angleW<<endl;

	double angleE=info.robotInformation[myID].engineRotation;
...
//	cout<<"敌方坐标"<<"("<<armyE.x<<","<<armyE.y<<")";
	...
	//武器瞄准
	double sumangle=angleofTwo+angleW;
	double chaangle=angleofTwo-angleW;

	double sumangle2=angleofTwo+angleE;
	double chaangle2=angleofTwo-angleE;
