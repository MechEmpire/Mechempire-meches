...
#include<fstream>
using namespace std;
ofstream fout("behind.txt",ios::out|ios::app);
...
Circle Obstacle_Zone[2] = { { OBSTACLE0_X, OBSTACLE0_Y, 149 }, { OBSTACLE1_X, OBSTACLE1_Y, 149 } };
...
void AdjustWeapon(RobotAI_Order& order, double r_weapon)
{
	/*double theta;
	...
	if (abs(theta - r_weapon) > 180)
	{
		...
		{
			order.wturn = 1;
			//fout << "逆旋转中，引擎：" << r_engine << "  目标角:" << theta << "\n";
		}
		else
		{
			order.wturn = -1;
			//fout << "顺旋转中，引擎：" << r_engine << "  目标角:" << theta << "\n";
		}
	}
	else
	{
...
{
	double theta;
	if (x == gme.circle.x && y > gme.circle.y)
		theta = -90;
	else if (x == gme.circle.x && y < gme.circle.y)
		theta = 90;
	else
	{
...
bool AdjustEngine(RobotAI_Order& order, double r_engine, int x, int y)
{
	...
	if (abs(r_engine - theta) <= 3)
	{
		//fout << "已对准，引擎：" << r_engine << "  目标角:" << theta << "\n";
		return false;
	}
	else{
		if (abs(theta - r_engine) > 180)
		{
			if (r_engine > theta)
			{
				order.eturn = 1;
...
void Strategy1(const RobotAI_BattlefieldInformation& info, int myID)
{
	...
	if (myID == 0)
	{
		...
		{
		case HIDE:
			if (self_Bullet > MIN_BULLET && Isinzone(Obstacle_Zone[0], gme.circle) && Distance(gme.circle,genermy.circle) > 200)
			{
				STATE = ATTACK;
				attack_flame = 0;
			}
	...
	else
	{
		...
		{
		case HIDE:
			if (self_Bullet > MIN_BULLET && Isinzone(Obstacle_Zone[1], gme.circle) && Distance(gme.circle, genermy.circle) > 200)
			{
				STATE = ATTACK;
				attack_flame = 0;
			}
			if (!info.arsenal[0].respawning_time && self_Bullet <= MIN_BULLET && gme.circle.x < OBSTACLE1_X && genermy.circle.x > OBSTACLE1_X)
				STATE = ARSENAL0;
			else if (!info.arsenal[1].respawning_time && self_Bullet <= MIN_BULLET && gme.circle.x > OBSTACLE0_X && genermy.circle.x < 683)
				STATE = ARSENAL1;
			break;
		case ATTACK:
			attack_flame++;
...
{
	if (c.x <= x + 10 && c.x >= x - 10 && c.y <= y + 10 && c.y >= y - 10)
		return true;
	else
		return false;
}
...
void Behind_Obstacle(RobotAI_Order& order, int myID)
{
	if (myID == 0)
	{
		...
		GotoPoint(order, OBSTACLE0_X - 130 * cos(theta), OBSTACLE0_Y - 130 * sin(theta));
	...
	else
	{
		...
		GotoPoint(order, OBSTACLE1_X - 130 * cos(theta), OBSTACLE1_Y - 130 * sin(theta));
	}
}
...
void RobotAI::Update(RobotAI_Order& order, const RobotAI_BattlefieldInformation& info, int myID)
{
	...
	To_enermy.x = gme.circle.x;
	To_enermy.y = gme.circle.y;
	...
	AdjustWeapon(order, info.robotInformation[myID].weaponRotation);
	Strategy1(info, myID);
	...
	//if (!gme.remainingAmmo)
	//{
	//	if (!info.arsenal[0].respawning_time)
	//		GotoPoint(order, ARSENAL0_X, ARSENAL0_Y);
	//	else if (!info.arsenal[1].respawning_time)
	//		GotoPoint(order, ARSENAL1_X, ARSENAL1_Y);
	//	else
	//		Behind_Obstacle(order, myID);
	//}
	...
	Bigger[1].r += 10;
	Bigger[0].r += 10;
	if (abs(gme.weaponRotation - To_enermy.rotation) <= 1)
	{
		//fout << "aimed";
		if (!HitTestBeamCircle(To_enermy, Bigger[0]))
		{
			//fout << "//not hit 0 ";
			if (!HitTestBeamCircle(To_enermy, Bigger[1]))
			{
				//fout << "//not hit 1";
				order.fire = 1;
			}
	...
	//if (a == 1)
	//{
	//	if (GotoPoint(order, 90, 600))
	//		a = 2;
	//}
	//else
	//{
	//	if (GotoPoint(order, 650, 340))
	//		a = 1;
	//}
	/*if (l == 上)
	{
	if (GotoPoint(order, 50, 50))
	l = 左;

	}
	if (l == 左)
	{
	if (GotoPoint(order, 50, 600))
	l = 下;

	}
	if (l == 下)
	{
	if (GotoPoint(order, 1300, 600))
	l = 右;

	}
	if (l == 右)
	{
	if (GotoPoint(order, 1300, 50))
	l = 上;
	...
	//	if (!l)
	//	{
	//		m = Pre_Wind_Obstacle(order);
	//		l++;
	//	}
	//	Wind_Obstacle(order, m);
	//	flame++;
	//}
}
