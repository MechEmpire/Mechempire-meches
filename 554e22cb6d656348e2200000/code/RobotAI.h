...
public:
	...
	void strategy1(RobotAI_Order& order,const RobotAI_BattlefieldInformation& info,int myID);
	bool fire(Beam b,const RobotAI_BattlefieldInformation& info);
	int against_position(int currentRun);
	int escape(Beam b,const RobotAI_BattlefieldInformation& info,int my_id,int currentRun);
	int Go(double current_x,double current_y,Point p);
	double GetDistance(Point x,Point y);
	double GetDistance(Point x,Circle y);
	bool isBullet(weapontypename a);
	int v_position(double x,double y);
	int e_position(double x1,double y1,double x2,double y2);
