...
void RobotAI::Update(RobotAI_Order& order,const RobotAI_BattlefieldInformation& info,int myID)
{
	...
	if(type==1)
	{
		if(min_x-50>info.robotInformation[myID].circle.x)
			order.run=2;
		else
			order.run=3;
	}
	...
	{
		if(min_x+50<info.robotInformation[myID].circle.x+info.robotInformation[myID].circle.r)
			order.run=1;
		else
			order.run=4;
	}
	...
	if(myID==1)
	{
		...
		mis.y=y5;
		mis.r=110;
		if(HitTestBeamCircle(shoot,mis))
			order.wturn=-1;
	}
	else
	{
